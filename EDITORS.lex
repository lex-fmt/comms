...
2. Lex-handling features
    ...
    Implementation status:
        ...
        | Add missing footnote definition [4]         | Ok      | Ok      | Ok           |
        | *Commands — conversion*                     |         |         |              |
...
3. Footnotes
    ...
    4. The "Add missing footnote definition" quickfix landed server-side in `lex-lsp` v0.8.6 (lex-fmt/lex#463). Each client surfaces it through its standard code-action flow — vscode and lexed via `vscode-languageclient` / `monaco-languageclient` respectively, nvim via `vim.lsp.buf.code_action`. Edit shape: append a new list item to an existing root- or session-scoped `:: notes ::` block when one exists, otherwise create a fresh `:: notes ::` block at end of document. Known limitation: Lex requires 2+ items to form a list, so a `:: notes ::` block with a single item does not re-parse as a footnote-definition list — the quickfix produces correct scaffolding but a single-footnote document needs the user to add a second item or accept that the reference will not resolve until the list grows.
    :: notes ::
