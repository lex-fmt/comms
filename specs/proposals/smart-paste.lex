...
4. The Re-anchor Transform
    ...
    4.4 The First Line
        ...
        Fresh-line case:
            The range start is on a blank or whitespace-only line (or at the auto-indented head of a new line). The whole paste is a new block. Every line, including the first, takes the §4.3 treatment. The first line is then reduced by any whitespace already present ahead of the range start — the auto-indent the editor left on the caret line that an empty range does not overwrite — so the anchor it carries is never stacked on top of that surviving whitespace. When the range *does* cover the leading whitespace (the editor expanded it, or the caret sat at column zero) nothing survives ahead of it and the full anchor applies; the two cases converge on the same anchored result. The one case the editor alone can fix is an auto-indent *deeper* than the anchor: an insert-only edit cannot remove the surplus, so an editor that wants it corrected downward expands the range to cover the line's leading whitespace (§2).
